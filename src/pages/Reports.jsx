  ...
  return (
    <div className="flex flex-col flex-auto px-5 pb-10">
      <div className="flex flex-col size-full max-w-5xl mx-auto">
        ...
        {/* Blog Filter */}
        <div
          className={`transition-opacity duration-400 ${isLoaded ? "" : "opacity-0"}`}
        >
        ...
        {/* Table Layout */}
        <table
          className={`mt-5 dark:bg-stone-900 bg-stone-200 rounded-2xl overflow-hidden hidden team2:table team3:text-base text-sm transition-opacity duration-400 ${isLoaded ? "" : "opacity-0"}`}
        >
          {/* Table Head */}
          <thead>
        ...
        {/* Cards Layout */}
        <div
          className={`mt-5 flex flex-col gap-5 team2:hidden transition-opacity duration-400 ${isLoaded ? "" : "opacity-0"}`}
        >
          {reports.map((report) => (
            <a
              key={report.key}
